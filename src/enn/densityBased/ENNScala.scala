...
//        for ( iteration <- 1 to _config.maxIterations ) 
        {
            ...
            val splitComputationNumber = Math.ceil(activeNodes / _config.maxComputingNodes.toDouble).toInt
...
//            : RDD[(TN, Iterable[TN])]
            ...
            graphKNN = exploded_graph.flatMap(tuple => 
            {
                 ...
                 */
//                if(tuple._1.getId.hashCode() % splitComputationNumber == iterationNumber % splitComputationNumber)
                if(computingNode(tuple._1.getId, iterationNumber, splitComputationNumber))
                {
                    val nodes : Set[TN] = tuple._2.toSet ++ Set(tuple._1)
//                    val nodes : Set[TN] = tuple._2.toSet.take(pruning) ++ Set(tuple._1)
                    
                   nodes.map(t => (t, neighborSimilarityInnerLoop(t, nodes, nodeManager_, nodeExcluded, neighborListFactory)))
                } else
                {
                    Array[(TN, NeighborList[TID, T, TN])]((tuple._1, neighborSimilarityInnerLoop(tuple._1, tuple._2, nodeManager_, nodeExcluded, neighborListFactory))) 
                }
            }).filter(t => !t._2.isEmpty()).cache
            
            graphKNN.cache().first();
            ...
            {
                // TODO we should collect ENN neighbors here 
                val neighborKNN = neighborListFactory.create(_config.kMax)
            ...
            graphENN = graphENNPlusExclusion.map(t => (t._1, t._2._1)).cache()
    ...
    {
        /*
         * we start with an empty ENN graph
         */
